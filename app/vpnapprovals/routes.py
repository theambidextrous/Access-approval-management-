...
from app.models import Approvevpn,VpnRequest
from app import db
import uuid
from app.utils import AuthUtil
from app.utils import SysAccess as sa

vpnapprovals = Blueprint('vpnapprovals', __name__)

# handle 404
@vpnapprovals.errorhandler(404)
def invalid_route(e):
...
# routes
# VPN APPROVALS
@vpnapprovals.route('/api/v0/requests/vpn/approve', methods = ['GET'])
...
def find_all(sess_instance_user):
    ...
    try:
        approvals = Approvevpn.query.all()
        if not approvals:
...
@vpnapprovals.route('/api/v0/requests/vpn/approve/byuser/<user_id>', methods = ['GET'])
@AuthUtil.auth_required
def find_all_by_user(sess_instance_user, user_id):
    if not sess_instance_user.isadmin and sess_instance_user.public_id != user_id:
        return jsonify({'status':-236, 'error':'Permission denied'})
    try:
        approvals = Approvevpn.query.filter_by(user_id=user_id)
        if not approvals:
            return jsonify({'status':0,'data':None})
        rtn = []
        for approval in approvals:
...
@vpnapprovals.route('/api/v0/requests/vpn/approve/byrequest/<request_id>', methods = ['GET'])
@AuthUtil.auth_required
def find_all_by_request(sess_instance_user, request_id):
    try:
        approvals = Approvevpn.query.filter_by(request_id=request_id)
        if not approvals:
            return jsonify({'status':0,'data':None})
        rtn = []
        for approval in approvals:
...
@vpnapprovals.route('/api/v0/requests/vpn/approve/<public_id>', methods = ['GET'])
@AuthUtil.auth_required
def find_one(sess_instance_user,public_id):
    try:
        approval = Approvevpn.query.filter_by(public_id=public_id).first()
        if not approval:
            return jsonify({'status':0,'data':None})
        rtn = []
        dt = {}
...
@vpnapprovals.route('/api/v0/requests/vpn/approve/byrequser', methods = ['GET'])
@AuthUtil.auth_required
def find_one_by_user_and_req(sess_instance_user):
    try:
        data = request.get_json()
        approval=Approvevpn.query.filter_by(request_id=data['request_id'], approved_by=sess_instance_user.public_id).first()
        if not approval:
...
@vpnapprovals.route('/api/v0/requests/vpn/approve/byrequser', methods = ['PUT'])
@AuthUtil.auth_required
def update_one_by_user_and_req(sess_instance_user):
    try:
        ...
        if not approval:
            return jsonify({'status':0,'data':None})
        approve_as = str(sess_instance_user.can_approve_vpn_as)
        if data['flag'] == '0':
            approve_as = '00'
...
@vpnapprovals.route('/api/v0/requests/vpn/approve', methods = ['POST'])
@AuthUtil.auth_required
def create_one(sess_instance_user):
    ...
    if sess_instance_user.can_approve_vpn_as == 0:
    ...
    try:
        ...
        if data['flag'] == '0':
            approve_as = '00'
            vrequest = VpnRequest.query.filter_by(request_id=data['request_id'],issubmitted=True).first()
            if not srequest:
                return jsonify({'status':-404, 'message':'Server resource not found'})
            vrequest.issubmitted = False
        if sess_instance_user.public_id == data['user_id']:
            return jsonify({'status':-236, 'error':'Self Approval Prohibited'})
        elif sess_instance_user.can_approve_vpn_as != int(data['approve_as']):
            return jsonify({'status':-236, 'error':'Invalid Values in payload '})
            ...
            approval = Approvevpn(public_id=str(uuid.uuid4()), request_id=data['request_id'], user_id=data['user_id'], approved_by=sess_instance_user.public_id, approve_as=approve_as, remarks=data['remarks'])
            db.session.add(approval)
            db.session.commit()
            sa.is_approved_vpn(data['request_id'])
